...
- Physical GPU self-test: creates shared GPU render targets, performs NV12 conversion, selects a hardware H.264 MFT, requests IDR and verifies actual Annex B IDR output. Does not capture desktop contents or install a virtual display.
...
Final command outcomes and hardware self-test resolution are recorded in the task's execution output. Build artifacts live under target/release and build/driver/Release and are not source-controlled.
