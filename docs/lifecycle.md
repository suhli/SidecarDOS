...
VideoToolbox's asynchronous jobs retain frame context and weakly reference the decoder. A private, non-actor session owner holds the native VTDecompressionSession and drains asynchronous callbacks before invalidating it on release. Reset and reconfiguration advance the decoder generation before releasing that owner; decoder destruction uses the same cleanup through stored-property release. The decoder no longer reads a non-Sendable native handle in an actor-isolated class's nonisolated deinit.

Metal retains CV texture views until its command buffer completes. On a physical iPad, presentation statistics use the drawable's presented handler and exclude skipped drawables with zero presentedTime. Simulator builds exclude those unavailable drawable APIs and use successful command-buffer completion as a timing estimate. Simulator statistics do not measure actual onscreen presentation latency. See Apple's [Metal Simulator guidance](https://developer.apple.com/documentation/metal/developing-metal-apps-that-run-in-simulator). The input UIView relinquishes held input when dismantled.
