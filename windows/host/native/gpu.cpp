...
void checked(HRESULT hr, const char *expression, int line)
{
    if (FAILED(hr))
    {
        sprintf_s(errorText, "%s (line %d): HRESULT 0x%08lx", expression, line,
...
struct Security
{
    ...
    Security()
    {
        check(ConvertStringSecurityDescriptorToSecurityDescriptorW(
                  L"D:P(A;;GA;;;SY)(A;;GA;;;LS)(A;;GA;;;UD)(A;;GA;;;OW)", SDDL_REVISION_1,
                  &descriptor, nullptr)
                  ? S_OK
...
struct Encoder
{
    ...
    void set(const GUID &key, ULONG value, bool required = true)
    {
        ...
        if (required && FAILED(hr))
        {
            sprintf_s(errorText, "ICodecAPI property %08lx, value %lu: 0x%08lx", key.Data1, value,
                      static_cast<unsigned long>(hr));
            throw hr;
        }
    }
    void flag(const GUID &key, bool enabled, bool required = true)
    {
        ...
        VariantInit(&v);
        v.vt = VT_BOOL;
        v.boolVal = enabled ? VARIANT_TRUE : VARIANT_FALSE;
        HRESULT hr = codec->SetValue(&key, &v);
        if (required)
            check(hr);
    }
    ...
    void activate(IMFActivate *activation, UINT bitrate)
    {
        ...
        check(transform->GetAttributes(&a));
        UINT32 aware = 0;
        check(a->GetUINT32(MF_SA_D3D11_AWARE, &aware));
        if (!aware)
            throw MF_E_UNSUPPORTED_D3D_TYPE;
        check(a->SetUINT32(MF_TRANSFORM_ASYNC_UNLOCK, TRUE));
        check(a->SetUINT32(MF_LOW_LATENCY, TRUE));
        check(transform.As(&codec));
        check(transform.As(&events));
        ...
        check(transform->GetStreamCount(&inCount, &outCount));
        if (inCount != 1 || outCount != 1)
            throw E_NOTIMPL;
        HRESULT ids = transform->GetStreamIDs(1, &inputId, 1, &outputId);
        if (ids == E_NOTIMPL)
        {
            inputId = outputId = 0;
        }
        ...
        media(in, MFVideoFormat_NV12);
        check(transform->SetInputType(inputId, in.Get(), 0));
    ...
    void init(UINT low, LONG high, UINT w, UINT h, UINT rate, UINT bitrate, const wchar_t *prefix)
    {
        ...
        check(device.As(&video));
        check(context.As(&videoContext));
        UINT token = 0;
        check(MFCreateDXGIDeviceManager(&token, &manager));
        check(manager->ResetDevice(device.Get(), token));
        ...
        Security security;
        SECURITY_ATTRIBUTES sa{sizeof(sa), security.descriptor, FALSE};
        ...
        for (UINT i = 0; i < 3; i++)
        {
            check(device->CreateTexture2D(&td, nullptr, &shared[i]));
            check(shared[i].As(&mutexes[i]));
            ComPtr<IDXGIResource1> resource;
            check(shared[i].As(&resource));
            auto name = std::wstring(prefix) + L"." + std::to_wstring(i);
            check(resource->CreateSharedHandle(
                &sa, DXGI_SHARED_RESOURCE_READ | DXGI_SHARED_RESOURCE_WRITE, name.c_str(),
                &handles[i].value));
        }
        ComPtr<IMFAttributes> attrs;
        check(MFCreateAttributes(&attrs, 1));
        UINT64 adapterId =
            static_cast<UINT64>(low) | (static_cast<UINT64>(static_cast<UINT32>(high)) << 32);
        check(attrs->SetUINT64(MFT_ENUM_ADAPTER_LUID, adapterId));
        MFT_REGISTER_TYPE_INFO input{MFMediaType_Video, MFVideoFormat_NV12},
            output{MFMediaType_Video, MFVideoFormat_H264};
    ...
    void pump()
    {
        for (UINT i = 0; i < 32; i++)
        {
            ...
            check(hr);
            HRESULT status = S_OK;
            check(event->GetStatus(&status));
            check(status);
            MediaEventType type;
            check(event->GetType(&type));
            if (type == METransformNeedInput)
                credits = std::min<UINT>(credits + 1, 3);
            else if (type == METransformHaveOutput)
                outputs = std::min<UINT>(outputs + 1, 4);
    ...
    HRESULT input(UINT slot, uint64_t frame, uint64_t capture)
    {
        if (slot >= 3)
            return E_INVALIDARG;
        MutexLease lease;
        lease.mutex = mutexes[slot];
        HRESULT acquired = lease.mutex->AcquireSync(1, 0);
        ...
        lease.owned = true;
        pump();
        if (frame == 0 || !credits || pending.size() >= 3)
            return S_OK;
        D3D11_TEXTURE2D_DESC td{};
        td.Width = width;
        td.Height = height;
        ...
        check(device->CreateTexture2D(&td, nullptr, &nv12));
        D3D11_VIDEO_PROCESSOR_INPUT_VIEW_DESC iv{};
        iv.ViewDimension = D3D11_VPIV_DIMENSION_TEXTURE2D;
        ComPtr<ID3D11VideoProcessorInputView> inputView;
        check(video->CreateVideoProcessorInputView(shared[slot].Get(), enumerator.Get(), &iv,
                                                   &inputView));
        D3D11_VIDEO_PROCESSOR_OUTPUT_VIEW_DESC ov{};
        ov.ViewDimension = D3D11_VPOV_DIMENSION_TEXTURE2D;
        ComPtr<ID3D11VideoProcessorOutputView> outputView;
        check(
            video->CreateVideoProcessorOutputView(nv12.Get(), enumerator.Get(), &ov, &outputView));
        ...
        check(lease.mutex->ReleaseSync(0));
        lease.owned = false;
        ComPtr<IMFMediaBuffer> buffer;
        check(MFCreateDXGISurfaceBuffer(__uuidof(ID3D11Texture2D), nv12.Get(), 0, FALSE, &buffer));
        ComPtr<IMFSample> sample;
        check(MFCreateSample(&sample));
        check(sample->AddBuffer(buffer.Get()));
        LONGLONG time = static_cast<LONGLONG>(capture * 10);
        check(sample->SetSampleTime(time));
        check(sample->SetSampleDuration(10000000 / fps));
        check(transform->ProcessInput(inputId, sample.Get(), 0));
        --credits;
        pending.emplace(time, std::make_pair(frame, capture));
        return S_OK;
    }
};
struct NativeOutput
{
    ...
    uint32_t keyframe, size;
};
} // namespace
extern "C" int32_t sd_gpu_create(uint32_t low, int32_t high, uint32_t w, uint32_t h, uint32_t fps,
                                 uint32_t bitrate, const wchar_t *prefix, void **out)
{
    if (!out || !prefix || w < 320 || h < 320 || w > 4096 || h > 4096 || (w & 1) || (h & 1) ||
        fps < 30 || fps > 60)
        return E_INVALIDARG;
    *out = nullptr;
    try
    {
        ...
        e->init(low, high, w, h, fps, bitrate, prefix);
        *out = e.release();
        return S_OK;
    }
    catch (HRESULT hr)
...
extern "C" void sd_gpu_destroy(void *p)
{
    delete static_cast<Encoder *>(p);
}
extern "C" int32_t sd_gpu_input(void *p, uint32_t slot, uint64_t frame, uint64_t capture)
{
    try
    {
        return static_cast<Encoder *>(p)->input(slot, frame, capture);
    }
    catch (HRESULT hr)
    {
        return hr;
...
extern "C" int32_t sd_gpu_bitrate(void *p, uint32_t bitrate)
{
    try
    {
        static_cast<Encoder *>(p)->set(CODECAPI_AVEncCommonMeanBitRate, bitrate);
        return S_OK;
    }
    catch (HRESULT hr)
    {
...
extern "C" int32_t sd_gpu_keyframe(void *p)
{
    try
    {
        static_cast<Encoder *>(p)->set(CODECAPI_AVEncVideoForceKeyFrame, 1);
        return S_OK;
    }
    catch (HRESULT hr)
    {
...
extern "C" int32_t sd_gpu_poll(void *p, uint8_t *bytes, uint32_t capacity, NativeOutput *out)
{
    try
    {
        auto &e = *static_cast<Encoder *>(p);
        e.pump();
        if (!e.outputs)
            return S_FALSE;
        MFT_OUTPUT_STREAM_INFO info{};
        check(e.transform->GetOutputStreamInfo(e.outputId, &info));
        ComPtr<IMFSample> allocated;
        ComPtr<IMFMediaBuffer> allocatedBuffer;
        if (!(info.dwFlags & MFT_OUTPUT_STREAM_PROVIDES_SAMPLES))
        {
            check(MFCreateSample(&allocated));
            check(MFCreateMemoryBuffer(std::max<DWORD>(info.cbSize, capacity), &allocatedBuffer));
            check(allocated->AddBuffer(allocatedBuffer.Get()));
        }
        MFT_OUTPUT_DATA_BUFFER b{};
        b.dwStreamID = e.outputId;
        b.pSample = allocated.Get();
        DWORD status = 0;
        HRESULT result = e.transform->ProcessOutput(0, 1, &b, &status);
        --e.outputs;
        if (b.pEvents)
            b.pEvents->Release();
        ComPtr<IMFSample> sample;
        if (b.pSample == allocated.Get())
            sample = allocated;
        else
            sample.Attach(b.pSample);
        if (result == MF_E_TRANSFORM_STREAM_CHANGE)
        {
            ComPtr<IMFMediaType> type;
            check(e.transform->GetOutputAvailableType(e.outputId, 0, &type));
            check(e.transform->SetOutputType(e.outputId, type.Get(), 0));
            return S_FALSE;
        }
        check(result);
        if (!sample)
        ...
        check(sample->GetSampleTime(&timestamp));
        auto entry = e.pending.find(timestamp);
        if (entry == e.pending.end())
            return E_UNEXPECTED;
        out->frame = entry->second.first;
        out->capture_us = entry->second.second;
        e.pending.erase(entry);
        UINT32 clean = 0;
...
extern "C" int32_t sd_gpu_self_test()
{
    try
    {
        ...
        wchar_t name[80]{};
        swprintf_s(name, L"Global\\SidecarDOS.Test.%08x%04x%04x", guid.Data1, guid.Data2,
                   guid.Data3);
        Encoder e;
        e.init(desc.AdapterLuid.LowPart, desc.AdapterLuid.HighPart, 1920, 1080, 60, 12000000, name);
        e.set(CODECAPI_AVEncVideoForceKeyFrame, 1);
        std::vector<uint8_t> output(2 * 1024 * 1024);
        NativeOutput meta{};
        uint64_t firstIdr = 0;
        bool sawSps = false, sawPps = false;
        for (uint64_t i = 1; i <= 180; i++)
        {
            ...
            if (acquired == S_OK)
            {
                ...
                check(e.device->CreateRenderTargetView(e.shared[slot].Get(), nullptr, &view));
                const float color[]{static_cast<float>(i % 60) / 60.0f, 0.2f, 0.4f, 1.0f};
                e.context->ClearRenderTargetView(view.Get(), color);
                e.context->Flush();
                check(e.mutexes[slot]->ReleaseSync(1));
                check(e.input(slot, i, i * 16667));
            }
            Sleep(10);
            HRESULT hr =
                sd_gpu_poll(&e, output.data(), static_cast<uint32_t>(output.size()), &meta);
            check(hr);
            if (hr == S_OK && meta.size > 0)
            {
                for (size_t j = 0; j + 4 < meta.size; j++)
                {
                    if (output[j] == 0 && output[j + 1] == 0 && output[j + 2] == 1)
                    {
                        ...
                        if (nal == 5)
                        {
                            if (firstIdr && meta.frame > firstIdr && sawSps && sawPps)
                                return S_OK;
                            firstIdr = meta.frame;
                            e.set(CODECAPI_AVEncCommonMeanBitRate, 6000000);
                            e.set(CODECAPI_AVEncVideoForceKeyFrame, 1);
                        }
                    }
                }
...
extern "C" const char *sd_gpu_error()
{
    return errorText;
}
