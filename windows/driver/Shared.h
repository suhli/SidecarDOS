...
constexpr DWORD SD_SURFACES = CTL_CODE(FILE_DEVICE_UNKNOWN,0x803,METHOD_BUFFERED,FILE_WRITE_DATA);
#pragma pack(push,8)
struct SdMode { uint32_t width,height,fps; };
struct SdStart { uint32_t abi,count; GUID identity; SdMode modes[SD_MODES]; };
struct SdSlot { uint64_t frame,capture_us; };
struct SdStatus {
 uint32_t abi,generation,width,height,fps,luid_low;
 int32_t luid_high; uint32_t reserved;
 SdSlot slots[SD_SLOTS];
};
struct SdSurfaces { uint32_t abi,generation; wchar_t names[SD_SLOTS][SD_NAME]; };
#pragma pack(pop)
