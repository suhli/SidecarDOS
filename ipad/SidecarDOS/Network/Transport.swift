...
@MainActor final class QUICTransport: SidecarTransport {
    ...
    // Network and TLS callbacks are explicitly scheduled on .main. Their Sendable
    // signatures do not express actor isolation; assumeIsolated checks that queue
    // contract synchronously, preserving packet order without creating extra tasks.
    func connect(_ endpoint: NWEndpoint, trusted: TrustedHost?) {
        ...
        group.stateUpdateHandler = { [weak self, weak group] state in
            MainActor.assumeIsolated {
                guard let self, let group, self.epoch == epoch else { return }
                switch state {
    ...
    private func openStreams(_ group: NWConnectionGroup, epoch: UInt64) {
        ...
        control.stateUpdateHandler = { [weak self, weak control] state in
            MainActor.assumeIsolated {
                guard let self, let control, self.epoch == epoch else { return }
        ...
        flow.stateUpdateHandler = { [weak self, weak flow] state in
            MainActor.assumeIsolated {
                guard let self, let flow, self.epoch == epoch else { return }
    ...
    func close() {
        epoch += 1
        control?.stateUpdateHandler = nil; input?.stateUpdateHandler = nil
        datagrams?.stateUpdateHandler = nil; group?.stateUpdateHandler = nil
        control?.cancel(); input?.cancel(); datagrams?.cancel(); group?.cancel()
